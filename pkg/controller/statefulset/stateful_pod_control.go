...
func (spc *realStatefulPodControl) createPersistentVolumeClaims(set *apps.StatefulSet, pod *v1.Pod) error {
	...
	for _, claim := range getPersistentVolumeClaims(set, pod) {
		...
		default:
			...
			changed := false
			// Because statefulSet object could be created without AccessModes, we need check out the lenth of AccessModes
			// When the lenth of AccessModes is zero, we do nothing.
